...
async function addClient(userName,password,email,firstName,lastName){
    var userRole='CLIENT';
    var sql=`INSERT INTO GENERAL_USER(USER_NAME, PASSWORD, EMAIL, FIRST_NAME, LAST_NAME , USER_ROLE)
    VALUES(:userName,:password,:email,:firstName,:lastName,:userRole)`;
    await database.simpleExecute(sql,{userName,password,email,firstName,lastName,userRole});
    await database.simpleExecute(`INSERT INTO CLIENT(USER_NAME) VALUES(:userName)`,{userName});
}
